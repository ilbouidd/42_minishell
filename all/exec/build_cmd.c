...
/*   Updated: 2026/09/02 10:17:09 by ilbouidd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_files *new_file_node(int type, char *path, int is_heredoc)
{
	t_files	*file;
	...
	file->mode = type;
	file->is_heredoc = is_heredoc;
	file->heredoc_content = NULL;
	file->next = NULL;
	return (file);
}

static void add_file_to_cmd(t_cmd *cmd, int type, char *path, int is_heredoc)
{
	t_files	*new;
	t_files	*tmp;

	new = new_file_node(type, path, is_heredoc);
	if (!new)
		return ;
	if (!cmd->files)
	{
	...
	*tmp = (*tmp)->next;
	...
	if (type == REDIR_HERE)
	{
		if (handle_heredoc(shell, path, quoted, &heredoc_file) != 0)
			return (0);
		add_file_to_cmd(cmd, REDIR_IN, heredoc_file, 1);
		free(heredoc_file);
	}
	else
		add_file_to_cmd(cmd, type, path, 0);
	*tmp = (*tmp)->next;
	return (1);
}
